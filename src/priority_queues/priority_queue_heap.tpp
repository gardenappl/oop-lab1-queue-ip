...
T priority_queue_heap<T>::pop()
{
	...
	while (true)
	{
		...
		if (left_index < data.size() && this->sort_comparator(data[max_index], data[left_index]))
			max_index = left_index;
		if (right_index < data.size() && this->sort_comparator(data[max_index], data[right_index]))
